...
class Share_whatsapp extends REST_Controller {
    ...
    public function index_post()
    {
      ...
      if(isset($headers['token']))
        ...
        if (empty($cektoken)) {
          ...
          $cekShare =  $this->mymodel->getbywhere('share_whatsapp',"date_share='$date' and relawan_id = $cektoken->relawan_id  and news_id =$news_id",null,'row');
          if (!empty($cekShare)) {
            $response =  array('status' => 0 , 'message' => 'Hari Ini Berita ini sudah anda bagikan ke whatsapp' );
          }else {
          $news_id = $this->post('news_id');
          $data = array(
            ...
            $in = $this->mymodel->insert('share_whatsapp',$data);
            $data = array(
              'relawan_id' => $cektoken->relawan_id,
              'poin' => 10,
              'keterangan' => "Share whatsapp",
              'created_at' => date('Y-m-d H:i:s')
            );
