...
class Transaksi_dokter extends CI_Controller {
  ...
  public function index($val = "")
  {
    ...
    $this->load->view('admin/data_transaksi_dokter');
  ...
  public function alldata()
  {
    ...
    $fetch_data = $this->Transaksi_dokter_datatable->make_datatables();
           ...
           {
            if ($value->is_aktif == "1") {
              $get_klinik = $this->mymodel->getbywhere('klinik',"is_deleted='0' and klinik_id=",$value->klinik_id,'row');
                $sub_array = array();
                $sub_array[] = $nomor;
                $sub_array[] = $value->nama_dokter;
                $sub_array[] = $get_klinik->nama_klinik;
                if ($get_klinik->status_pembayaran == "1") {
                  $sub_array[] = "LUNAS";
                  $convert = date("d m Y N",strtotime($get_klinik->tanggal_pembayaran));
                  ...
                  $convert = date("d m Y N",strtotime($get_klinik->tanggal_expired));
                  $convert = explode(" ", $convert);
                  $tgl = $convert[0];
                  $bln = $this->get_bulan($convert[1]);
                  $thn = $convert[2];
                  ...
                  $sub_array[] = $tgl_bayar;
                  $sub_array[] = "Rp. ".number_format($get_klinik->biaya,0,"",".");
  ...
  public function insertdata()
  {
    ...
    if ($_REQUEST['status_pembayaran'] == "1") {
     ...
     if(!empty($data)){
      ...
      redirect('admin/owner/transaksi_dokter/');
  }
  ...
  public function deletedata()
  {
    ...
    $del = $this->mymodel->update('trans_dokter',$data_transaksi_dokter,'trans_dokter_id',$_REQUEST['data_id']);
    if ($del) {
