...
class Notifikasi_asisten_antrian extends CI_Controller {
  ...
  public function index($val = "")
  {
    ...
    $this->load->view('admin/data_notifikasi_asisten_antrian');
    $this->load->view('admin/footer');
  }
  public function alldata()
  {
    $this->load->model(array('Notifikasi_antrian_datatable'));
    $fetch_data = $this->Notifikasi_antrian_datatable->make_datatables();
           $data = array();
           $nomor=1;
           foreach($fetch_data as $value)
           {
            $get_asisten_data = $this->mymodel->getbywhere('asisten_dokter','username',$this->session->userdata('asisten'),'row');
            if ($value->dokter_id == $get_asisten_data->dokter_id and $value->status_antrian == "0") {
           ...
           $output = array(
                ...
                "recordsTotal"          =>      $this->Notifikasi_antrian_datatable->get_all_data(),
                "recordsFiltered"     =>     $this->Notifikasi_antrian_datatable->get_filtered_data(),
                "data"                    =>     $data
           );
  ...
  {
    $get_asisten_data = $this->mymodel->getbywhere('asisten_dokter','username',$this->session->userdata('asisten'),'row');
    $get_antrian_pasien = $this->mymodel->getbywhere('antrian',"dokter_id='".$get_asisten_data->dokter_id."' and status_antrian=","0","result");
      ...
      redirect('admin/asisten/notifikasi_asisten_antrian/');
  }
  ...
  public function getdataedit()
  {
    ...
    $this->load->view('admin/modal_edit/notifikasi_asisten_antrian',$data);
  }
