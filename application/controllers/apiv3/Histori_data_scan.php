...
class Histori_data_scan extends REST_Controller {
  ...
  public function get_hari($h){
    ...
    public function index_get()
    {
      ...
      $msg="";
      if ($token!='') {
        $tanggal = date("Y-m-d",strtotime($this->get('date')))." 23:59:59";
        $pasien = $this->mymodel->getbywhere('pasien','token',$token,'row');
        $data = $this->mymodel->withquery("select histori_data_scan_id,dokter_id,created_at from histori_data_scan where pasien_id='".$pasien->pasien_id."' and is_deleted='0' and created_at < '".$tanggal."' order by created_at DESC, klinik_id ASC",'result');
        ...
        foreach ($data as $key => $value) {
          ...
          if ($this->get('type') == "1") {
            $value->type = "Klinik Berlangganan";
          }
          else if($this->get('type') == "2"){
            $value->type = "Klinik Tidak Berlangganan";
          }
