                          ...
                          var token = '<?php //echo $mydata->token ?>';
                           ...
                           refku.on("value", function(data) {
                               <?php if (isset($_REQUEST['id'])): ?>
                                 $.post( "<?php echo site_url('admin/asisten/chat/chat_list_side?id='.$_REQUEST['id']) ?>",{})
                                 .done(function( data ) {
                                 //  console.log(list);
                                     $('#chat_room').html(data);
                                 });
                               <?php else: ?>
                                 $.post( "<?php echo site_url('admin/asisten/chat/chat_list_side') ?>",{})
                                 .done(function( data ) {
                                    //console.log(list);
                                     $('#chat_room').html(data);
                                 });
...
<!-- main-panel ends -->
  ...
  <?php if (isset($_REQUEST['id'])): ?>
        $.post( "<?php echo site_url('admin/asisten/chat/reloadchat?id='.$_REQUEST['id']) ?>",{
        })
        .done(function( data ) {
  ...
  $(document).ready(function(){
       ...
       var dataTable = $('#data').DataTable({
         ...
         "buttons": {
            ...
            "order":[],
            "ajax":{
                 url:"<?php echo base_url() .'admin/asisten/jadwal_dokter/alldata/'; ?>",
                 type:"POST"
            },
            "columnDefs":[
                 {
       ...
       $(document).on("click", ".delete", function(e) {
        ...
        $(document).on('click', '.update', function(){
            var a = $(this).attr("id");
            $.post('<?php echo site_url('admin/asisten/jadwal_dokter/getdataedit') ?>',
            {id :a},function (data) {
              if (data!="" || data != null) {
                $('#editform').html(data);
                $('#btneditdata').click();
         ...
         refku.on("value", function(data) {
          ...
          $('#msg').on('keypress', function (e) {
                   if(e.which === 13){
                      ...
                      //Do Stuff, submit, etc..
                      $.post( "<?php echo site_url('admin/asisten/chat/insertchat') ?>",{
